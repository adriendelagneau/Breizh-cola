...
import BreizhOriginal from './experience/view/BreizhOriginal';
  ...
  useEffect(() => {
    if (timeline2 && screenRef.current) {
      ScrollTrigger.create({
        ...
        start: 'top 30%', // Adjust the trigger point as needed
...
<BreizhOriginal />
      ...
      >
        <MagneticButtons>
          <button className="text-mainColor dark:text-secondColor rotate-12 border-mainColor  h-[75px] sm:h-[100px] sm:w-[200px] text-lg sm:text-xl md:text-2xl 2xl:w-[360px] xl:w-[300px] xl:h-[150px] 2xl:h-[180px] xl:text-4xl uppercase rounded-[50%] cursor-pointer w-[150px]  border-2 font-bold hover:text-secondColor  md:w-[250px] md:h-[125px] hover:bg-mainColor dark:hover:text-mainColor dark:hover:bg-secondColor dark:border-secondColor 2xl:-rotate-6">
            <TransitionLink href={"/products/breizh-cola-original"} label={"decouvrez le"}/>
          </button>
        </MagneticButtons>
      </div>
      ...
      <div className="flex flex-col gap-12 ml-3 text-3xl uppercase sm:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl font-poppins">
        <p className="flex gap-4 overflow-hidden">
          <span ref={ref1} className="translate-y-full text-mainColor dark:text-secondColor">
            la recette
          </span>
          <span
            ref={ref2}
            className="overflow-hidden translate-y-full text-stroke-1 text-stroke-mainColor text-secondColor dark:text-mainColor dark:text-stroke-secondColor"
          >
            originale
          </span>
        </p>
        <p className="flex gap-1 overflow-hidden">
          <span
            ref={ref3}
            className="translate-y-full text-stroke-1 text-stroke-mainColor text-secondColor dark:text-stroke-secondColor dark:text-mainColor"
          >
            Son
          </span>
          <span ref={ref4} className="overflow-hidden translate-y-full text-mainColor dark:text-secondColor">
            secret ?
          </span>
        </p>
        <p className="flex gap-4 mt-12 overflow-hidden text-3xl sm:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl">
          <span ref={ref5} className="translate-y-full text-mainColor dark:text-secondColor">
            le dosage
          </span>
          <span
            ref={ref6}
        ...
        <p className="flex gap-8 mt-2 overflow-hidden text-3xl sm:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl">
          <span ref={ref7} className="translate-y-full text-mainColor dark:text-secondColor">
            des ingredients
          </span>
        </p>
      </div>
