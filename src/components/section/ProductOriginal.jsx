...
import { useGSAPTimeline2 } from "@/store/zuStore";  // Import the timeline hook from your store
...
const ProductOriginal = () => {
  ...
  useGSAP(() => {
    if (timeline && screenRef.current) {
      // Create the ScrollTrigger for the animation
      ScrollTrigger.create({
        trigger: screenRef.current,
        start: 'top 50%', // Adjust the trigger point as needed
        once: true,       // Ensure the animation only happens once
        onEnter: () => {
          // Use the existing timeline from the store
          timeline
            .to(
              [
                ref1.current,
  ...
  }, [timeline]); // Dependency on the timeline
